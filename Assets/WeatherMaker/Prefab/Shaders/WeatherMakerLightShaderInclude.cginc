...
// legacy built in render pipeline only, not URP compatible
...
#define WEATHER_MAKER_SPECCUBE_LOD_STEPS UNITY_SPECCUBE_LOD_STEPS
#define WeatherMakerBoxProjectedCubemapDirection BoxProjectedCubemapDirection

#else

// URP cannot include the built-in Standard shader library. Weather Maker only
// needs this constant and box-projection helper from that library.
#define WEATHER_MAKER_SPECCUBE_LOD_STEPS 6.0

inline float3 WeatherMakerBoxProjectedCubemapDirection(float3 worldRefl, float3 worldPos, float4 cubemapCenter, float4 boxMin, float4 boxMax)
{
	UNITY_BRANCH
	if (cubemapCenter.w > 0.0)
	{
		float3 reflectionDirection = normalize(worldRefl);
		float3 intersectionMax = (boxMax.xyz - worldPos) / reflectionDirection;
		float3 intersectionMin = (boxMin.xyz - worldPos) / reflectionDirection;
		float3 intersection = (reflectionDirection > 0.0) ? intersectionMax : intersectionMin;
		float distanceToBox = min(min(intersection.x, intersection.y), intersection.z);

		worldRefl = (worldPos - cubemapCenter.xyz) + (reflectionDirection * distanceToBox);
	}
	return worldRefl;
}
...
fixed3 ComputeReflectionColor(float3 worldPos, float3 rayDir, float3 normal, float2 screenUV, fixed4 specular)
{
	...
	if (specular.a <= 0.0)
	{
		...
		specular.a = saturate(WEATHER_MAKER_SPECCUBE_LOD_STEPS * specular.a);
	}
	...
	reflVector = WeatherMakerBoxProjectedCubemapDirection(reflVector, worldPos, unity_SpecCube0_ProbePosition, boxMin, boxMax);
