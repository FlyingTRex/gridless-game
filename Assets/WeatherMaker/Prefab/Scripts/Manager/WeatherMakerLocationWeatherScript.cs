...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerLocationWeatherScript : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (AutoStartStop)
            {
                WeatherMakerInputHelper.StartLocationService(AccuracyMeters, ThresholdMeters);
            }
        ...
        private void OnDisable()
        {
            if (AutoStartStop)
            {
                WeatherMakerInputHelper.StopLocationService();
            }
        }
        ...
        private void Update()
        {
            if (WeatherApi != null && ApiKey != null && WeatherZoneScript != null)
            {
                ...
                if (ElapsedTime >= UpdateRateSeconds)
                {
                    ...
                    else if ((Latitude > 998.0f || Longitude > 998.0f) && WeatherMakerInputHelper.TryGetLocation(out lat, out lon))
                    {
                    }
                    else
                    {
