        ...
        , IWeatherMakerNetworkConnection
    {
        ...
        public string GetConnectionId(Transform obj)
        {
            ...
            if (obj != null)
            {
                ...
                {
                    // NetworkConnection.connectionId doesn't exist on the
                    // Mirror version installed in this project (Weather
                    // Maker's script targets an older Mirror API) --
                    // GetHashCode() is a stand-in unique-per-connection
                    // string, not the real sync ID scheme. This whole
                    // method is unreachable in the current single-player
                    // build anyway (out of scope per WEATHER_MAKER_PLANNING.md
                    // section 6's decided feature scope) -- revisit for
                    // real when actual multiplayer weather sync gets built
                    // (see MULTIPLAYER_PLANNING.md).
                    return id.connectionToClient.GetHashCode().ToString(CultureInfo.InvariantCulture);
                }
                else
                {
                    return id.connectionToServer.GetHashCode().ToString(CultureInfo.InvariantCulture);
                }
            }
