...
9. **Cooking** — 🟡 Gardening's 16-cell grid built (v0.3.79-dev), 7 crops with real seed-packet models (v0.3.80-dev), 6 of 7 crops grow through real Wild Harvest growth-stage art (v0.3.81-dev), and now have real harvested-pickup visuals too (v0.3.83-dev, 2026-08-15) — only Corn's visuals stay placeholder throughout (not in the pack). The 4 Campfire cooking accessories (Grill/Cooking Pot/Kettle/Frying Pan) now have real models, icons, and Forging-skill recipes too (v0.3.90-dev, 2026-08-15) — previously blank-placeholder/admin-spawn-only. **All 4 accessories now have at least one recipe** (v0.3.91-dev through v0.3.94-dev, 2026-08-15): Grilled Meat (Grill, Herb+Raw Meat), Steak and Potatoes (Frying Pan, Raw Meat+Potato), Herbal Tea (Kettle, Herb+Water — the first recipe needing water, which added `CookableItem.requiresCanteenWater`/`Campfire.HasCanteenWater` as a new mechanic), Meat Stew (Cooking Pot, Raw Meat+Potato+Carrot+Water, the most complex recipe yet) — each with its own real merged model, no placeholders. **Cooking's own skill/quality-tier system is also built** (v0.3.93-dev, `COOKING_SKILL_PLANNING.md`) — all 4 new recipes above roll a real success/fail outcome via `CraftOutcomeRoll`, gated behind Cooking skill level (5/5/15/25 respectively), with a mild Health hit on the worst roll; the original Raw Meat → Cooked Meat recipe stays free/risk-free. Only real gap left in this item: wild-forage seed sourcing (still admin-spawn only).
