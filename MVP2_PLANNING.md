...
9. **Cooking** — 🟡 Gardening's 16-cell grid built (v0.3.79-dev), now 7 crops with real seed-packet models (v0.3.80-dev, 2026-08-15) — Carrot/Potato/Corn/Ginger/Turnip/Onion/Sweet Potato, growing-plant visuals still placeholder primitives, Admin-Spawn-only seed sourcing (wild forage nodes not built). Cooking's own skill/quality-tier system (`CookableItem` gaining `trainedSkill`/`CraftOutcomeRoll`) still not built.
