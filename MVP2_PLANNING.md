...
2. **Expand NPC hiring** beyond stonework (Mining is the only job family today). — 🟡 Woodcutting + Gathering shipped (v0.3.32-dev). Bench-crafting (Metalworking pilot) planned in full (2026-08-16, `NPC_JOB_GENERALIZATION_PLANNING.md` section 7) — new `NPCCrafting.cs` sibling component, `Furnace`-style recipe queue, deterministic (no risk roll), not yet built. Same session, two more directly-related pieces also planned in full: **NPC training** (`NPC_TRAINING_PLANNING.md`) — a Desk/Bookshelf ritual that grants a trained NPC the same recipe exception a skill book gives the player, feeding straight into the bench-crafting queue above — and a **Village Flag** (`VILLAGE_FLAG_PLANNING.md`) that spawns and draws in new hireable NPCs, frequency scaled by Fame band + the Flag's own crafted tier. None of the three are built yet. Guarding still unstarted/unplanned.
