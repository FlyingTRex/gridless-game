...
9. **Cooking** — 🟡 Gardening's 16-cell grid built (v0.3.79-dev), 7 crops with real seed-packet models (v0.3.80-dev), 6 of 7 crops now grow through real Wild Harvest: Root Vegetables 12-stage art instead of placeholders (v0.3.81-dev, 2026-08-15) — only Corn's visual stays placeholder. Admin-Spawn-only seed sourcing still open (wild forage nodes not built). Cooking's own skill/quality-tier system (`CookableItem` gaining `trainedSkill`/`CraftOutcomeRoll`) still not built.
