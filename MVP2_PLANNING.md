...
2. **Expand NPC hiring** beyond stonework (Mining is the only job family today). — 🟡 Woodcutting + Gathering shipped (v0.3.32-dev). The full "Settlement Growth Loop" built 2026-08-16 (v0.3.101-dev through v0.3.105-dev, `NPC_JOB_GENERALIZATION_PLANNING.md` section 7, `NPC_TRAINING_PLANNING.md`, `VILLAGE_FLAG_PLANNING.md`): bench-crafting (Metalworking pilot — `NPCCrafting.cs`/`NPCCraftingScreen.cs`, `Furnace`-style recipe queue, deterministic, pilot `MetalworkingJob`/`IronIngotRecipe`; other families data-only follow-ups, not built), NPC training (Desk/Bookshelf ritual granting a trained NPC the same recipe exception a skill book gives the player, feeding straight into the bench-crafting queue), Village Flag spawn loop (`VillageFlagSpawner.cs`, frequency scaled by Fame band + Flag tier, now nameable with its name shown on the Player Map), and the City Statue Village→City gate (`CityStatue.cs`, Masterwork Flag + 10 hired NPCs live precondition, +50 Fame, reusable `requiresCityStatus` flag nothing yet consumes). All verified via compile + YAML only — **nothing in this whole arc has been live-tested in Play mode yet**, and two pieces (Village Flag spawn, City Statue) need real time/setup to test at all (a real spawn takes up to 30 minutes at baseline; the Statue needs a Masterwork Flag + 10 actual hires). Guarding still unstarted/unplanned.
