...
9. **Cooking** — 🟡 Gardening's 16-cell grid built (v0.3.79-dev), 7 crops with real seed-packet models (v0.3.80-dev), 6 of 7 crops grow through real Wild Harvest growth-stage art (v0.3.81-dev), and now have real harvested-pickup visuals too (v0.3.83-dev, 2026-08-15) — only Corn's visuals stay placeholder throughout (not in the pack). The 4 Campfire cooking accessories (Grill/Cooking Pot/Kettle/Frying Pan) now have real models, icons, and Forging-skill recipes too (v0.3.90-dev, 2026-08-15) — previously blank-placeholder/admin-spawn-only. 3 of the 4 now also have their first accessory-gated `CookableItem` (v0.3.91-dev/v0.3.92-dev, 2026-08-15): Grilled Meat (Grill, Herb+Raw Meat), Steak and Potatoes (Frying Pan, Raw Meat+Potato, own merged model), Herbal Tea (Kettle, Herb+Water — the first recipe needing water, which added `CookableItem.requiresCanteenWater`/`Campfire.HasCanteenWater` as a new mechanic). Cooking Pot still has zero recipes. Admin-Spawn-only seed sourcing still open (wild forage nodes not built). **Cooking's own skill/quality-tier system is now built too** (v0.3.93-dev, 2026-08-15, `COOKING_SKILL_PLANNING.md`) — the 3 gated recipes above now roll a real success/fail outcome via `CraftOutcomeRoll`, gated behind Cooking skill level, with a mild Health hit on the worst roll; the original Raw Meat → Cooked Meat recipe stays free/risk-free. Only real gaps left in this item: Cooking Pot's missing recipe, and wild-forage seed sourcing.
