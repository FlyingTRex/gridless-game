...
Hit live (2026-08-14, Berry Seed's world-pickup model): Ben reported Admin
Spawn "isn't working" for Berry Seed specifically, while Berry (a
different item) spawned and rendered correctly through the identical
`AdminSpawnScreen`/`PlayerDropping.SpawnPickup` code path. The
`worldPickupPrefab` reference, `Pickup` component, and mesh/collider setup
all checked out fine via YAML — the actual cause was one level deeper:
`BerrySeed.glb`'s embedded material (auto-generated by the glTFast
importer, shown in the Inspector as shader `Shader Graphs/glTF...`) never
got extracted into a real, standalone `.mat` asset with a proper
`Universal Render Pipeline/Lit` shader the way `Assets/Data/BerryPickup
.mat` (Berry's own working material) already was. Same end symptom as the
legacy Built-in-shader HumanDummy bug from earlier this session — a
shader that isn't truly URP-compatible can render fully invisible instead
of the usual pink "shader missing" indicator — just a different root
mechanism (an unconverted *imported* material vs. a hand-authored legacy
one).

**Not a project-wide pattern — checked, not assumed.** A sweep of every
`Assets/Models/*.glb.meta` file for an `externalObjects` material remap
(the marker a model's embedded material was ever extracted/replaced)
found **zero** other models with one, including dozens that have been
visibly working fine all session (Pickaxe, Campfire, the Ingots,
Trimmed Stick, etc.). So "no remap" does not by itself mean "broken" —
most models' embedded materials happen to render correctly as-is,
apparently specific to how each one's source textures/colors came out at
generation/export time. This was a one-off on Berry Seed specifically,
not a symptom worth a wider audit.

**The fix — extract the material via script, not just the Editor's
"Extract Materials" button** (this session runs headless):
```csharp
var mat = new Material(Shader.Find("Universal Render Pipeline/Lit"));
mat.SetColor("_BaseColor", /* appropriate color */);
AssetDatabase.CreateAsset(mat, "Assets/Data/YourModelPickup.mat");

var importer = AssetImporter.GetAtPath(modelPath); // NOT ModelImporter —
// glTFast's importer is GLTFast.Editor.GltfImporter, a ScriptedImporter,
// not Unity's built-in FBX/OBJ ModelImporter. AddRemap/SaveAndReimport
// are on the base AssetImporter class either way, so this works
// regardless of which concrete importer type it turns out to be.
foreach (var obj in AssetDatabase.LoadAllAssetsAtPath(modelPath))
{
    if (obj is Material embeddedMat)
        importer.AddRemap(new AssetImporter.SourceAssetIdentifier(embeddedMat), mat);
}
importer.SaveAndReimport();
```
Verify by grepping the model's own `.meta` file for the new
`externalObjects` entry (not the prefab or scene — the remap lives on the
*model's* importer settings) and confirming the guid matches the new
`.mat` asset, same "don't trust the log" discipline as every other
asset-creation gotcha in this file. A render screenshot (same
`RenderTexture`/`ReadPixels` pattern `IconBaker.cs` uses) is worth the
extra step too, specifically because this bug class's whole signature is
"looks fine structurally, renders invisible" — a passing YAML check alone
doesn't prove the object is actually visible.

**How to apply:** if a specific imported model (not a whole material/
shader family) reports as invisible/not-appearing while sibling items
using the same spawn/render code path work fine, suspect the model's own
embedded material before the surrounding game code — check the Inspector
